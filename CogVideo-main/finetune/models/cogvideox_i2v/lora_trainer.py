...
class CogVideoXI2VLoraTrainer(Trainer):
    ...
    def load_components(self) -> Dict[str, Any]:
        ...
        components.transformer.init_quaternion_modules()
        ...
        model_path = 'weights/typhoon/best_model.pth'
        # model_path = '/code/weights/fluid/best_model.pth'

        flow_model = vit_base_patch16_384().cuda()
        flow_model_path = 'weights/typhoon_flow/best_model.pth'
        ...
        components.domain_model = domain_model
        components.flow_model = flow_model
    ...
    def initialize_pipeline(self) -> CogVideoXImageToVideoPipeline:
        pipe = CogVideoXImageToVideoPipeline(
            ...
            domain_model=self.components.domain_model,
    ...
    def encode_image(self, image: Image.Image) -> torch.Tensor:
        ...
        with torch.no_grad():
            ...
            domain_images = image.unsqueeze(0)

            # domain_images = batch["pixel_values"][:,0,:,:,:]
            domain_images = F.interpolate(domain_images, size = [384,384], mode = "bilinear")
            domain_features = self.components.domain_model(domain_images)
            flow_features = self.components.flow_model(domain_images)
        
        # shape of domain_features: [1, C]
        # shape of flow_features: [1, C]
    ...
    def collate_fn(self, samples: List[Dict[str, Any]]) -> Dict[str, Any]:
        ret = {"encoded_videos": [], "prompt_embedding": [], "images": [], "domain_features": [], "flow_features": []}
        ...
        for sample in samples:
            ...
            ret["domain_features"].append(sample["domain_features"])
            ret["flow_features"].append(sample["flow_features"])

        ret["encoded_videos"] = torch.stack(ret["encoded_videos"])
        ret["prompt_embedding"] = torch.stack(ret["prompt_embedding"])
        ret["images"] = torch.stack(ret["images"])
        ret["domain_features"] = torch.stack(ret["domain_features"])
        ret["flow_features"] = torch.stack(ret["flow_features"])
    ...
    def compute_loss(self, batch) -> torch.Tensor:
        ...
        images = batch["images"]
        domain_features = batch["domain_features"]
