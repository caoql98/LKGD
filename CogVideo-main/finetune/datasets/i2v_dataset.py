...
class BaseI2VDataset(Dataset):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if hasattr(trainer, "encode_image"):
    ...
    def __getitem__(self, index: int) -> Dict[str, Any]:
        ...
        if encoded_video_path.exists():
            saved_data = load_file(encoded_video_path)
            encoded_video = saved_data["encoded_video"]
            domain_features = saved_data["domain_features"]
            flow_features = saved_data["flow_features"]
            ...
            domain_features, flow_features = self.encode_image(image)
            domain_features, flow_features = domain_features.to("cpu"), flow_features.to("cpu")

            # [1, C, F, H, W] -> [C, F, H, W]
            encoded_video = encoded_video[0]
            encoded_video = encoded_video.to("cpu")
            image = image.to("cpu")
            save_file({"encoded_video": encoded_video, "domain_features": domain_features, "flow_features": flow_features}, encoded_video_path)
            logger.info(f"Saved encoded video to {encoded_video_path}", main_process_only=False)


            # Save domain features and flow features
