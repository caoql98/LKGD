...
from utils.optical_flow import optical_flow_latent_unnormalize, optical_flow_squeeze
...
class StableVideoDiffusionPipelineControlNet(DiffusionPipeline):
    ...
    def decode_latents(self, latents, num_frames, decode_chunk_size=14):
        ...
        forward_vae_fn = self.vae.forward
    ...
    def __call__(
        ...
        # controlnet_condition = torch.cat([controlnet_condition, image_latents], dim=2)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                #     torch.ones_like(t) * np.log(1e3) * 0.25,
        ...
        if not output_type == "latent":
            ...
            if needs_upcasting:
                self.vae.to(dtype=torch.float16)
            unnormalized_latents = optical_flow_latent_unnormalize(latents)
            frames = self.decode_latents(unnormalized_latents, num_frames, decode_chunk_size)
