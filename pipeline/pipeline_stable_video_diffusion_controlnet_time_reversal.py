...
class StableVideoDiffusionPipelineControlNet(DiffusionPipeline):
    ...
    def __call__(
        ...
        image_embeddings2 = self._encode_image(image2, device, num_videos_per_prompt, do_classifier_free_guidance)
        image_embeddings = torch.cat([image_embeddings, image_embeddings2], dim=0)
        ...
        image2 = self.image_processor.preprocess(image2, height=height, width=width)
        ...
        image_latents = image_latents[[0,2,1,3]]
        ...
        controlnet_condition = torch.flip(controlnet_condition, dims=[1])
        # 8. Denoising loop
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                # if i % 2:
                #     cur_image_latents = image_latents[len(latent_model_input):]
                #     cur_image_embeddings = image_embeddings[len(latent_model_input):]
                # else:
                #     cur_image_latents = image_latents[:len(latent_model_input)]
                #     cur_image_embeddings = image_embeddings[:len(latent_model_input)]
                cur_image_latents = image_latents[len(latent_model_input):]
        ...
        #         latents = torch.flip(latents, dims=[1])
        #         # image_embeddings = torch.flip(image_embeddings, dims=[1])
        #         # image_latents = torch.flip(image_latents, dims=[1])
        #         controlnet_condition = torch.flip(controlnet_condition, dims=[1])
        # if len(timesteps) % 2:
        #     latents = torch.flip(latents, dims=[1])
