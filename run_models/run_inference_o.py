...
if __name__ == "__main__":
    args = {
        ...
        "validation_image_folder": "data/test_images/video_trans_input/3762907-uhd_3840_2160_25fps.mp4",
        # "validation_control_folder": "/data/juicefs_sharing_data/72179586/repos/VidToMe/outputs/tea-pour/depth_image",
        # "validation_image": "/data/juicefs_sharing_data/72179586/repos/VidToMe/outputs/tea-pour/vector/frames/0000.png",
        "output_dir": "./output_dir/depth_control",
        "height": 320,
        "width": 576,
        "num_frames": 25
    ...
    validation_images = load_input(args["validation_image_folder"])
    validation_images = process_frames(validation_images, h = args["height"], w = args["width"], verbose = True, div = 64)

    validation_control_images = prepare_control("depth", validation_images, 
                                                frame_ids= list(range(len(validation_images))), 
                                                save_path=os.path.join("output_dir", "depth_control"))

    validation_control_images = [T.ToPILImage()(img) for img in validation_control_images]
    # validation_image = Image.open(args["validation_image"]).convert('RGB')


    # Load and set up the pipeline
    controlnet = controlnet = ControlNetSDVModel.from_pretrained("CiaraRowles/temporal-controlnet-depth-svd-v1",subfolder="controlnet", cache_dir = "/data/vjuicefs_ai_camera_lgroup/11162591/gsy_workdir/hf_models_backup/")
    ...
    video_frames = pipeline(validation_images[start_f], validation_control_images[start_f:start_f+n_frames], decode_chunk_size=8,num_frames=n_frames,motion_bucket_id=127, fps = 25, controlnet_cond_scale=1.0,width=args["width"],height=args["height"]).frames
    ...
    save_gifs_side_by_side(video_frames,validation_images[start_f:], validation_control_images[start_f:],val_save_dir)
