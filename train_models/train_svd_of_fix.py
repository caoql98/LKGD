...
from models.unet_spatio_temporal_condition_flow import UNetSpatioTemporalConditionModelFlow
from pipeline.pipeline_stable_video_diffusion_flow_fix import StableVideoDiffusionPipelineFlow
...
from utils.util import _get_add_time_ids, denormalize_image, load_lora_weights, normalize_image, tensor_to_vae_latent
...
def parse_args():
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "whether to fix motion_bucket_id = 127, fps = 7"
        ),
    )
    # parser.add_argument(
    #     "--lora_rank",
...
def main():
    ...
    unet_class = UNetSpatioTemporalConditionModelFlow
    ...
    unet.initialize_conv_in()
    pipeline_class = StableVideoDiffusionPipelineFlow
    ...
    # train_modules.update({"conv_in2": unet.conv_in2, "conv_in2_alpha": unet.conv_in2_alpha})
    train_modules = {"conv_in2": unet.conv_in2, "conv_in2_alpha": unet.conv_in2_alpha}
    ...
    if args.resume_model is not None:
        ...
        state_dict = torch.load(os.path.join(input_dir, f"unet_trainable.pth"))
        unet.load_state_dict(state_dict["state_dict"], strict=False)
    ...
    if accelerator.is_main_process:
        ...
        rec_txt1 = open('train_svd_of_fix.txt', 'w')
        rec_txt2 = open('train_svd_of_fix_train.txt', 'w')
        for name, para in unet.named_parameters():
    ...
    def encode_image(image):
        ...
        assert image.min() >= 0, "Image values must be non-negative"
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(cur_train_dataloader):
            ...
            with accelerator.accumulate(unet):
                ...
                pred_flow = inference_flow_warpper(flow_model, raw_video, raw_video.shape[-2:])
                ...
                #     accelerator.device, non_blocking=True
                # )
                pred_flow = rearrange(pred_flow, "B T C H W -> (B T) C H W")
                flow_image = flow_to_image(pred_flow) / 255.
                flow_image = normalize_image(flow_image)
                flow_image = rearrange(flow_image, "(B T) C H W -> B T C H W", B=pixel_values.shape[0]).to(pixel_values)
                ...
                cond_image = torch.cat([flow_image[:, 0, :, :, :], pixel_values[:, 0, :, :, :]], dim = 0)
                ...
                conditional_latents = torch.cat(conditional_latents.chunk(2, dim=0), dim=1)
                ...
                raw_image = denormalize_image(pixel_values[:, 0, :, :, :])
                # Get the text embedding for conditioning.
                encoder_hidden_states = encode_image(raw_image)

                if args.fix_motion_bucket_id:
            ...
            if accelerator.sync_gradients:
                ...
                if accelerator.is_main_process:
                    ...
                    if (
                        ...
                        # validation_images_flow = flow_to_image_naive(validation_control_flows)

                        motion_bucket_id = cal_motion_bucket_ids(flow_model, validation_images.unsqueeze(0), torch.tensor([7]))
                        ...
                        with torch.autocast(
                            ...
                            for val_img_idx in range(args.num_validation_images):
                                ...
                                flow_frame = T.ToPILImage()(validation_control_images[0])
                                # flow_cond  = T.ToPILImage()(validation_images_flow[0])
                                flow_images = pipeline(
                                    cond_frame,
                                    height=args.height,
                                    width=args.width,
                                ...
                                save_gifs_side_by_side([flow_frames, validation_images[:14], validation_control_images[:14]],val_save_dir, global_step = global_step)
