...
from utils.dataset import DAVIS, MiniDataset, WebVid10M, Panda, compute_flow, optical_flow_normalize, process_frames, readBatchedFlow, readFlow
...
from utils.motion_helper import cal_motion_bucket_ids, flow2motion
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=int,
        default=10,
        help=("The repetition of the input video in one epoch."),
    )
    parser.add_argument(
        "--lora_name",
        type=str,
...
def main():
    ...
    unet_class = UNetSpatioTemporalConditionModel

    unet = unet_class.from_pretrained(
        args.pretrained_model_name_or_path if args.pretrain_unet is None else args.pretrain_unet,
        subfolder="unet",
        # low_cpu_mem_usage=True,
        low_cpu_mem_usage=False,  # 确保可以加载所有的权重
        device_map=None
        # variant="fp16",
    )
    ...
    for name, param in unet.named_parameters():
        if 'quaternion' in name:
            param.requires_grad_(True)  # 如果参数名中包含 'quaternion'，则设置 requires_grad 为 True
            # print(name)
        else:
            param.requires_grad_(False)  # 其他参数设置为 False
    ...
    unet_lora_config = LoraConfig(
        ...
        layers_pattern="temporal_transformer_blocks.*",
        target_modules=["attn1.to_k", "attn1.to_q", "attn1.to_v"],
    )
    ...
    if version.parse(accelerate.__version__) >= version.parse("0.16.0"):
        ...
        #             unet = model
        #     save_dict = dict()
        #     save_dict["state_dict"] = dict()
        #     # for name, module in train_modules.items():
        #     #     state_dict[name] = module.state_dict()
        #     # torch.save(state_dict, os.path.join(output_dir, "unet.pth"))
        #     # save_dict = {"state_dict": dict()}
        #     for name, para in unet.named_parameters():
        #         if para.requires_grad is True:
        #             # print(name)
        #             save_dict["state_dict"][name] = para.detach().clone()
        #     if len(save_dict["state_dict"]) > 0:
        #         torch.save(save_dict, os.path.join(output_dir, f"unet_trainable.pth"))
    ...
    params = lora_layers
    param_to_name = {param: name for name, param in unet.named_parameters()}
    ...
    # 3. 遍历 optimizer.param_groups，输出参数的名称和形状
    for i, group in enumerate(optimizer.param_groups):
        print(f"Param group {i}:")
        for param in group['params']:
            param_name = param_to_name.get(param, "Unknown parameter")
    ...
    if accelerator.is_main_process:
        ...
        rec_txt1 = open('train_svd_lora.txt', 'w')
        rec_txt2 = open('train_svd_lora_train.txt', 'w')
        for name, para in unet.named_parameters():
    ...
    domain_model = vit_base_patch16_384().cuda()
    # model_path = '/code/multimodal-prompt-learning/premodels/B2_vitb16_mae_ep99.pth'
    # model_path = '/code/MAE-ViT-pytorch-main/MAE-ViT-pytorch-main/fluidxyz_logs1/best_model.pth'
    model_path = '/code/weights/typhoon/best_model.pth'
    # model_path = '/code/weights/fluid/best_model.pth'

    flow_model = vit_base_patch16_384().cuda()
    flow_model_path = '/code/weights/typhoon_flow/best_model.pth'
    # flow_model_path = '/code/weights/fluid_flow/best_model.pth'
    ...
    domain_model.load_state_dict(encoder_weights)


    pretrained_flow_weights = torch.load(flow_model_path)
    encoder_flow_weights = {}
    for key in pretrained_flow_weights.keys():
        if key.startswith('encoder.'):
            new_key = key[len('encoder.'):]
            encoder_flow_weights[new_key] = pretrained_flow_weights[key]
    flow_model.load_state_dict(encoder_flow_weights)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(cur_train_dataloader):
            ...
            with accelerator.accumulate(unet):
                ...
                with torch.no_grad():
                    domain_images = rearrange(batch["pixel_values"], "B T C H W->(B T) C H W")
                    # domain_images = batch["pixel_values"][:,0,:,:,:]
                    domain_images = F.interpolate(domain_images, size = [384,384], mode = "bilinear")
                    domain_features = domain_model(domain_images)
                    domain_features = rearrange(domain_features, "(B T) C ->B T C ", B=batch["pixel_values"].shape[0])
                    domain_features = domain_features.mean(dim=1,keepdim=True)
                    # domain_features = domain_features.unsqueeze(1)

                    flow_features = flow_model(domain_images)
                    flow_features = rearrange(flow_features, "(B T) C ->B T C ", B=batch["pixel_values"].shape[0])
                    flow_features = flow_features.mean(dim=1,keepdim=True)


                # motion_bucket_ids = [flow2motion(fl.norm(dim=-3).mean(), fps) for fl, fps in zip(pred_flow, batch["fps"])]
                raw_video = denormalize_image(batch["pixel_values"])
                # motion_bucket_ids = cal_motion_bucket_ids(flow_model, raw_video, batch["fps"])
                # # print(motion_bucket_ids)
                # fps = batch["fps"] - 1
                # print(motion_bucket_ids)
                motion_bucket_ids = 127
                fps = 5
                ...
                # controlnet_image = rearrange(controlnet_image, "B T C H W->(B T) C H W")
                # flow_latents = F.interpolate(controlnet_image, size = latents.shape[-2:], mode = "bilinear")
                # flow_latents = rearrange(flow_latents, "(B T) C H W->B T C H W", B=latents.shape[0])
                # controlnet_image = optical_flow_normalize(controlnet_image)
                # flow_latents = optical_flow_expand(flow_latents)
                ...
                # cond_image = flow_image[:, 0, :, :, :]

                init_frame_noise = torch.randn_like(cond_image)
                small_noise_init_frame = cond_image + init_frame_noise * train_noise_aug

                conditional_latents = vae.encode(small_noise_init_frame).latent_dist.mode()
                # conditional_latents = optical_flow_latent_normalize(conditional_latents, scale = vae.config.scaling_factor)
                ...
                raw_image = denormalize_image(pixel_values)
                ...
                # encoder_hidden_states_list = [encoder_hidden_states, domain_feature]

                # Predict the noise residual
                model_pred = unet(
                    inp_noisy_latents, timesteps, encoder_hidden_states, domain_features,flow_features,
                    added_time_ids=added_time_ids,
            ...
            if accelerator.sync_gradients:
                ...
                if accelerator.is_main_process:
                    ...
                    if global_step % 40 == 0:
                        ...
                        for name, param in unet_lora_state_dict.items():
                            print(f"Parameter Name: {name}, Shape: {param.shape}")

                        StableDiffusionPipeline.save_lora_weights(
                            save_directory=os.path.join(save_path, lora_name), 
                    ...
                    if (
                        (global_step % 10 == 0)
                        or (global_step - start_global_step == 1) 
                    ):
                        logger.info(
                        ...
                        validation_images = read_image(args.validation_image).to(weight_dtype).to(
                            accelerator.device, non_blocking=True
                        ).unsqueeze(0)
                        # validation_control_images = load_images_from_folder(args.validation_control_folder)
                        # validation_control_flows = compute_flow(validation_images.permute(0, 2, 3, 1))

                        # validation_control_flows = load_flow_from_folder(args.validation_control_folder)
                        # validation_images_tensor = torch.stack([T.ToTensor()(img) for img in validation_images])
                        # validation_control_flows = compute_flow(validation_images_tensor.permute(0, 2, 3, 1))
                        ...
                        # validation_control_images = F.interpolate(validation_control_images, size = validation_latents.shape[-2:], mode = "bilinear")
                        # validation_control_images = F.interpolate(validation_control_images, size = validation_control_flows.shape[-2:], mode = "bilinear")
                        ...
                        with torch.autocast(
                            ...
                            for val_img_idx in range(args.num_validation_images):
                                ...
                                # flow_cond  = T.ToPILImage()(validation_images_flow[0])
                                frames = pipeline(
                                    cond_frame,
                                    height=args.height,
                                    width=args.width,
                                ...
                                save_gifs_side_by_side([frames],val_save_dir, global_step = global_step)
