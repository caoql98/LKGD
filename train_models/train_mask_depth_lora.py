...
from utils.util import load_condition_latents, load_lora_weights, tensor_to_vae_latent, normalize_image
...
def main():
    ...
    if args.dataset_name is not None:
        ...
        dataset = load_image_folder(args.train_data_dir, args.cache_dir)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                # xy_input_ids = torch.cat([batch["input_ids"]] * 2, dim=0)

                # pdb.set_trace()
                pixel_values = batch["y_pixel_values"]
                # pixel_values = normalize_image(pixel_values)
                ...
                bsz = latents.shape[0]
                # rand_grid_size = torch.randint(grid_size_range[0], grid_size_range[1], (bsz,), device=latents.device)
                # rand_mask = [get_guassian_2d_rand_mask(gs, noise_patch_size) for gs in rand_grid_size]
                # rand_mask = torch.stack(rand_mask, dim = 0).unsqueeze(1)
                # rand_mask = F.interpolate(rand_mask, size = latents.shape[-2:], mode="bilinear")
                ...
                # Dropout 10% text-conditioning and 25% mask everything. Align with stable-diffusion-inpaint.
                if args.prompt_dropout_prob is not None:
                    random_p = torch.rand(
                        bsz, device=latents.device)
                    # Sample masks for the edit prompts.
                    prompt_mask = random_p < args.prompt_dropout_prob
                ...
                if args.mask_dropout_prob is not None:
                    ...
                    mask_mask = random_p < args.mask_dropout_prob
                ...
                masked_pixel_values = pixel_values * (rand_mask < 0.5)
            ...
            if accelerator.sync_gradients:
                ...
                if args.validation_prompt is not None and (step % args.validation_steps == 0 or initial_step):
                    ...
                    mask_images = [T.ToPILImage()(rm) for rm in rand_masks]
                    ...
                    prompts = [args.validation_prompt] * (batch_size)
                    ...
                    image_grid = make_image_grid([init_image, *mask_images, *images], rows=1, cols=1 + len(mask_images) + len(images))
                    ...
                    # pdb.set_trace()
    # accelerator.init_trackers("depth_exp")
