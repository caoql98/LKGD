...
from utils.util import blip_cap, denormalize_image, load_condition_latents, load_lora_weights, tensor_to_vae_latent
...
def make_train_dataset(args):
    ...
    dataset = Panda(args.train_data_dir, sample_size = (args.height, args.width), sample_n_frames = args.num_frames + 14, return_end_frames = True)
...
def main():
    ...
    patch.apply_patch(unet, single_dir = args.single_dir)
    ...
    patch.set_joint_layer_requires_grad(unet, trainable_loras, True)
    ...
    def collate_fn(examples):
        pixel_values_f0 = torch.cat([example["pixel_values"][::2] for example in examples], dim = 0)
        pixel_values_f1 = torch.cat([example["pixel_values"][1::2] for example in examples], dim = 0)
        pixel_values = torch.cat([pixel_values_f1, pixel_values_f0], dim = 0)
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                if accelerator.is_main_process and step % 200 == 0:
                    images = [transforms.ToPILImage()(denormalize_image(im)) for im in batch["pixel_values"]]
