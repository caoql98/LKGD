...
from pipeline.pipeline_stable_diffusion_inpaint_guidance_add_cond import StableDiffusionInpaintGuidanceAddCondPipeline
...
import diffusers
from diffusers import AutoencoderKL, DDPMScheduler, DiffusionPipeline, StableDiffusionPipeline, UNet2DConditionModel, StableDiffusionInpaintPipeline
from models.unet_2d_condition_cond import UNet2DConditionAddCondModel
...
from utils.util import load_condition_latents, load_lora_weights, tensor_to_vae_latent
from utils.dataset import process_frames, TrackDataset, PandaN
...
def parse_args():
    ...
    parser.add_argument(
        "--cond_image_track_path",
        type=str,
        default=None,
    )

    parser.add_argument(
        "--cond_depth_track_path",
        type=str,
        default=None,
    )
...
def main():
    ...
    unet_class = UNet2DConditionAddCondModel

    unet = unet_class.from_pretrained(args.pretrained_model_name_or_path, subfolder="unet", revision=args.revision, variant=args.variant, low_cpu_mem_usage = False, device_map = None)

    unet.zero_add_module()

    pipeline_class = StableDiffusionInpaintGuidanceAddCondPipeline
    ...
    patch.initialize_joint_layers(unet, post = args.post_joint, add_norm = args.add_joint_norm)
    ...
    if args.with_add_cond:
        # pdb.set_trace()
        for param in unet.add_cond_embedding.parameters():
    ...
    elif args.dataset_type == "PandaN":
        dataset = PandaN(dataset_file = args.train_data_dir, sample_size = args.resolution)
    ...
    def tokenize_captions(examples, is_train=True, trigger_word = ""):
        captions = []
        ...
        if y_caption_column is not None:
            y_captions = [caption for caption in examples[y_caption_column]]
        else:
            y_captions = captions
    ...
    def preprocess_train(examples, only_caption = False):
        ...
        if not only_caption:
            y_images = [image.convert("RGB") for image in examples[image_column]]
            x_images = [image.convert("RGB") for image in examples[x_column]]
            # pdb.set_trace()
        ...
        input_ids = tokenize_captions(examples, trigger_word = args.trigger_word)
    ...
    def collate_fn(examples):
        # pdb.set_trace()
        if len(examples[0]["x_pixel_values"].shape) == 3:
            x_pixel_values = torch.stack([example["x_pixel_values"] for example in examples], dim = 0)
            y_pixel_values = torch.stack([example["y_pixel_values"] for example in examples], dim = 0)
        elif len(examples[0]["x_pixel_values"].shape) == 4:
            x_pixel_values = torch.cat([example["x_pixel_values"] for example in examples], dim = 0)
            y_pixel_values = torch.cat([example["y_pixel_values"] for example in examples], dim = 0)
        ...
        if len(examples[0]["x_input_ids"].shape) == 1:
        ...
        elif len(examples[0]["x_input_ids"].shape) == 2:
            x_input_ids = torch.cat([example["x_input_ids"] for example in examples])
            y_input_ids = torch.cat([example["y_input_ids"] for example in examples])

        ret_dict = {"x_pixel_values": x_pixel_values, "x_input_ids": x_input_ids, "y_input_ids": y_input_ids, "y_pixel_values": y_pixel_values}

        if "x_tracks" in examples[0]:
            x_tracks = torch.stack([example["x_tracks"] for example in examples])
            x_tracks = x_tracks.to(memory_format=torch.contiguous_format).float()
            y_tracks = torch.stack([example["y_tracks"] for example in examples])
            y_tracks = y_tracks.to(memory_format=torch.contiguous_format).float()

            ret_dict["x_tracks"] = x_tracks
            ret_dict["y_tracks"] = y_tracks
        return ret_dict
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                xy_tracks = torch.cat([batch["x_tracks"], batch["y_tracks"]], dim=0) if "x_tracks" in batch else None
                ...
                add_cond = xy_tracks.to(input_data) if args.with_add_cond else None
                # Predict the noise residual and compute loss
                model_pred = unet(input_data, timesteps, encoder_hidden_states, return_dict=False, add_cond = add_cond)[0]
                
                if args.only_image_loss:
            ...
            if accelerator.sync_gradients:
                ...
                if args.validation_prompt is not None and (global_step % args.validation_steps == 0 or initial_step):
                    ...
                    init_image_track = PIL.Image.open(args.cond_image_track_path).convert("RGB")
                    init_depth_track = PIL.Image.open(args.cond_depth_track_path).convert("RGB")
                    init_tracks = [init_image_track, init_depth_track]
                    init_tracks = process_frames(init_tracks, args.resolution, args.resolution)
                    init_tracks = [init_tracks[0] for i in range(batch_size // 2)] + [init_tracks[1] for i in range(batch_size // 2)]
                    ...
                    add_cond = init_tracks if args.with_add_cond else None
                    with autocast_ctx:
                        for _ in range(args.num_validation_images):
                            images += pipeline(
                                    ...
                                    generator=generator,
                                    y_advance = 1.0,
                                    add_cond = init_tracks
                                ).images

                    # mask_images_upscale = process_frames(mask_images, height, width)
                    image_grid = make_image_grid([*init_images, *mask_images, *images], rows=3, cols=batch_size)
