...
from patch import patch_cond as patch
from utils.util import load_lora_weights
from utils.guassian_2d import gaussian_random_field, get_guassian_2d_rand_mask
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type = int,
        default=-1,
        help=("Fix noise seed. If > 0, use a fixed noise."),
    )
...
def main():
    ...
    if accelerator.is_main_process:
        ...
        rec_txt1 = open('rec_para_unet_lora.txt', 'w')
        rec_txt2 = open('rec_para_unet__lora_train.txt', 'w')
    ...
    logger.info(f" Grid size {grid_size} and noise patch size {noise_patch_size}")
        ...
        # logger.info(f" Using fixed noise with seed {args.fix_noise} and shape {fix_noise.shape}")
        # thresh_range = np.linspace(-3, 3, args.max_train_steps)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                # if args.fix_noise > 0:
                    # noise = fix_noise.
                    # noise = torch.stack([noise] * latents.shape[0]).to(latents)
                    # rand_mask = torch.randint(0, 2, size = (latents.shape[0],1,4,4))
                    # rand_mask = rand_mask.repeat_interleave(noise.shape[-1] // 4, dim = -1).repeat_interleave(noise.shape[-2] // 4, dim = -2)
                    # pad = [noise.shape[-2] - rand_mask.shape[-2]] * 2 + [noise.shape[-1] - rand_mask.shape[-1]] * 2
                    # rand_mask = F.pad(rand_mask, pad = pad, mode="constant", value = 0)
                    # rand_mask = rand_mask.expand_as(noise)
                    # noise[~rand_mask] = torch.randn_like(latents)[~rand_mask]
                rand_mask = get_guassian_2d_rand_mask(grid_size, noise_patch_size)
                rand_mask = rand_mask[...,:latents.shape[-2],:latents.shape[-1]]
                raw_unet = unwrap_model(unet)
                # rand_mask = rand_mask[None,None,...].expand_as(latents)
                # rand_mask = rand_mask[:,0]

                # patch.set_patch_lora_mask(raw_unet, "y_lora", [0,1,0,1])
                patch.set_patch_lora_mask(raw_unet, "yx_lora", rand_mask)
                patch.set_patch_lora_mask(raw_unet, "xy_lora", ~rand_mask)
                ...
                model_pred = model_pred[~rand_mask]
                target = target[~rand_mask]
            ...
            if args.validation_prompt is not None and epoch % args.validation_epochs == 0:
                ...
                with autocast_ctx:
                    for _ in range(args.num_validation_images):
                        images += pipeline(args.validation_prompt, num_images_per_prompt = 2, num_inference_steps=30, generator=generator, ).images
