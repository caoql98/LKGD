...
def main():
    ...
    pipeline_class = StableDiffusionInpaintControlNetPipeline

    logger.info("Initializing controlnet weights from unet")

    controlnet_class = ControlNetModel
    controlnet = controlnet_class.from_unet(unet, conditioning_channels = args.conditioning_channels)
    ...
    if args.train_lora:
        ...
        controlnet.add_adapter(controlnet_lora_config, adapter_name = "y_lora")
        controlnet.set_adapter(["y_lora"])
        trainable_loras = ["y_lora"]
        set_adapters_requires_grad(controlnet, True, trainable_loras)
    ...
    # if args.train_y:
    #     trainable_loras = ["y_lora", "xy_lora", "yx_lora"]
    # else:
    ...
    dataset = dataset.cast_column(x_column, Image())
    ...
    def tokenize_captions(examples, is_train=True):
        ...
        y_captions = [args.trigger_word + cpt for cpt in captions]
        captions += y_captions
        inputs = tokenizer(
    ...
    if accelerator.is_main_process:
        ...
        rec_txt1 = open('rec_para_controlnet.txt', 'w')
        rec_txt2 = open('rec_para_controlnet_train.txt', 'w')
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(controlnet):
                ...
                x_pixel_values = batch["x_pixel_values"]
                y_pixel_values = batch["y_pixel_values"]
                # x_input_ids = batch["x_input_ids"]
                # pixel_values = batch["y_pixel_values"]
                # input_ids = batch["input_ids"]


                # Convert images to latent space
                latents = vae.encode(x_pixel_values.to(dtype=weight_dtype)).latent_dist.sample()
                ...
                height, width = x_pixel_values.shape[-2], x_pixel_values.shape[-1]
                grid_size = max(height, width) // noise_patch_size + 1
                rand_mask = get_rand_masks(bsz * 2, grid_size, noise_patch_size = noise_patch_size)
                # rand_mask = torch.stack(rand_mask, dim = 0)
                rand_mask = rand_mask[...,:height, :width].expand(bsz * 2, 1, height, width).to(latents)
                ...
                if args.prompt_dropout_prob is not None:
                    ...
                    prompt_mask = prompt_mask.reshape(bsz * 2, 1, 1)
                    # Final text conditioning.
                    null_conditioning = torch.zeros_like(encoder_hidden_states)
                    encoder_hidden_states = torch.where(
                        prompt_mask, null_conditioning, encoder_hidden_states)
                
                if args.mask_dropout_prob is not None:
                    random_p = torch.rand(
                        bsz, device=latents.device)
                    # Sample masks for the original images.
                    x_mask_mask = random_p < 2 * args.mask_dropout_prob
                    y_mask_mask = torch.logical_and(random_p > args.mask_dropout_prob, random_p < 3 * args.mask_dropout_prob)
                    mask_mask = torch.cat([x_mask_mask, y_mask_mask], dim = 0)
                    mask_mask = mask_mask.reshape(bsz * 2, 1, 1, 1)
                    # Final image conditioning.
                    full_mask = torch.ones_like(rand_mask)
                    rand_mask = torch.where(
                        mask_mask, full_mask, rand_mask)

                    y_mask_zeromask = random_p < 0.5 * args.mask_dropout_prob
                    x_mask_zeromask = torch.logical_and(random_p > 2.5 * args.mask_dropout_prob, random_p < 3 * args.mask_dropout_prob)
                    mask_zeromask = torch.cat([x_mask_zeromask, y_mask_zeromask], dim = 0)
                    mask_zeromask = mask_zeromask.reshape(bsz * 2, 1, 1, 1)
                    zero_mask = torch.zeros_like(rand_mask)
                    rand_mask = torch.where(
                        mask_zeromask, zero_mask, rand_mask)
                ...
                x_rand_mask, y_rand_mask = rand_mask.chunk(2, dim = 0)
                x_masked_pixel_values = x_pixel_values * (x_rand_mask < 0.5)
                y_masked_pixel_values = y_pixel_values * (y_rand_mask < 0.5)
                x_rand_mask = torch.nn.functional.interpolate(
                    x_rand_mask, size=(height // vae_scale_factor, width // vae_scale_factor)
                ).to(latents)
                masked_image_latents = vae.encode(x_masked_pixel_values.to(dtype=weight_dtype)).latent_dist.sample()
                masked_image_latents = masked_image_latents * vae.config.scaling_factor
                input_data = torch.cat([noisy_latents, x_rand_mask, masked_image_latents], dim = 1)

                controlnet_cond = torch.cat([y_masked_pixel_values, y_rand_mask], dim = 1)

                x_encoder_hidden_states, y_encoder_hidden_states = encoder_hidden_states.chunk(2, dim = 0)
                ...
                model_pred = unet(
                    input_data,
                    timesteps,
                    encoder_hidden_states=x_encoder_hidden_states,
            ...
            if accelerator.sync_gradients:
                ...
                train_loss = 0.0
                if args.validation_prompt is not None and (step % args.validation_steps == 0 or initial_step):
                    initial_step = False
                    logger.info(
                    ...
                    prompts = [prompt if i < (batch_size // 2) else args.trigger_word + prompt for i in range(batch_size)]
                    with autocast_ctx:
                    ...
                    image_grid = make_image_grid([*init_images, *mask_images, *images, *images], rows=3, cols=batch_size)
                    ...
                    for tracker in accelerator.trackers:
                        ...
                        if tracker.name == "wandb":
                            tracker.log(
                                {
                                    "validation": [
                                        wandb.Image(image_grid, caption=f"{0}: {prompt}")
                                    ]
                                }
                            )
                ...
                if global_step % args.checkpointing_steps == 0:
                    if accelerator.is_main_process:
                        ...
                        for lora_name in trainable_loras:
                            ...
                            controlnet_lora_state_dict = convert_state_dict_to_diffusers(
                                get_peft_model_state_dict(unwrapped_controlnet, adapter_name=lora_name)
                            )
    ...
    if accelerator.is_main_process:
        ...
        state_dict = dict()
        for name, params in unwrapped_controlnet.named_parameters():
            if params.requires_grad:
                state_dict[name] = params
