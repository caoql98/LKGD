...
from run_inference import save_gifs_side_by_side
...
from utils.optical_flow import inference_flow_warpper, load_unimatch
from utils.util import _get_add_time_ids, denormalize_image, tensor_to_vae_latent
# Will error if the minimal version of diffusers is not installed. Remove at your own risks.
check_min_version("0.24.0.dev0")

logger = get_logger(__name__, log_level="INFO")

MODEL_SAVE = {"ControlNetSDVModel"}
...
def main():
    ...
    controlnet_class = ControlNetSDVModel

    if args.controlnet_model_name_or_path:
        logger.info("Loading existing controlnet weights")
        controlnet = controlnet_class.from_pretrained(args.controlnet_model_name_or_path)
    else:
        logger.info("Initializing controlnet weights from unet")
        controlnet = controlnet_class.from_unet(unet, conditioning_channels=args.conditioning_channels)

    if args.resume_model is not None:
        controlnet_state_dict = load_file(args.resume_model)
        controlnet.load_state_dict(controlnet_state_dict)
        print(f"Resume controlnet model from {args.resume_model}")
    ...
    # if version.parse(accelerate.__version__) >= version.parse("0.16.0"):
        ...
        #             model.save_pretrained(os.path.join(output_dir, f"{SAVE_NAMES[model.__class__.__name__]}"))
        #             return
        ...
        #             load_model = controlnet_class.from_pretrained(
        #                 input_dir, subfolder=SAVE_NAMES[controlnet_class.__name__])
    ...
    optimizer, lr_scheduler, train_dataloader,controlnet = accelerator.prepare(
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(cur_train_dataloader):
            ...
            with accelerator.accumulate(controlnet):
                ...
                controlnet_image = inference_flow_warpper(flow_model, denormalize_image(batch["pixel_values"]), pixel_values.shape[-2:])
                ...
                inp_noisy_latents = inp_noisy_latents.to(weight_dtype)
                added_time_ids = added_time_ids.to(weight_dtype)
                controlnet_image = controlnet_image.to(weight_dtype)
            ...
            if accelerator.sync_gradients:
                ...
                if accelerator.is_main_process:
                    ...
                    if (
                        ...
                        validation_images = load_images_from_folder_tensor(args.validation_image_folder)[:args.num_frames + 1]
                        ...
                        with torch.autocast(
                            ...
                            for val_img_idx in range(args.num_validation_images):
                                ...
                                video_frames = pipeline(
                                    validation_images[0], 
                                    validation_control_flows[:num_frames],
                                    height=args.height,
                                    width=args.width,
                                ...
                                save_gifs_side_by_side(video_frames,validation_images[:14], validation_control_images[:14],val_save_dir)
