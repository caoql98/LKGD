...
def main():
    ...
    all_conds = ["depth", "normal"]

    for cond in all_conds:
        unet.add_adapter(unet_lora_config, adapter_name = f"xy_lora_{cond}")
        unet.add_adapter(unet_lora_config, adapter_name = f"yx_lora_{cond}")

        all_loras += [f"xy_lora_{cond}", f"yx_lora_{cond}"]
    ...
    for cond in all_conds:
        unet.add_adapter(y_lora_config, adapter_name = f"y_lora_{cond}")
        all_loras += [f"y_lora_{cond}"]
    ...
    image_columns = [args.image_column, args.image_column2]
    trigger_words = [args.trigger_word, args.trigger_word2]

    cur_cond = random.randint(0, len(all_conds) - 1)
    ...
    def preprocess_train(examples):
        ...
        y_images = [image.convert("RGB") for image in examples[image_columns[cur_cond]]]
        ...
        input_ids = tokenize_captions(examples, trigger_word = trigger_words[cur_cond])
        x_len = len(input_ids) // 2
        examples["x_input_ids"] = input_ids[:x_len]
        examples["y_input_ids"] = input_ids[x_len:]
        examples["cond_id"] = [cur_cond for i in range(len(x_images))]
        return examples
    ...
    def collate_fn(examples):
        ...
        assert len(set([example["cond_id"] for example in examples])) == 1, "Cond id error"
        return {"x_pixel_values": x_pixel_values, "x_input_ids": x_input_ids, "y_input_ids": y_input_ids, "y_pixel_values": y_pixel_values, "cond_id": examples[0]["cond_id"]}
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                # random next cond
                cur_cond = random.randint(0, len(all_conds) - 1)
                # get cond id for this batch
                cond_id = batch["cond_id"]

                
                unet.module.set_adapters([lora_name for lora_name in all_loras if all_conds[cond_id] in lora_name])
                # print(cond_id)
                # print([lora_name for lora_name in all_loras if all_conds[cond_id] in lora_name])
