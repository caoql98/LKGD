...
from utils.peft_utils import get_peft_model_state_dict, set_adapters_requires_grad, set_peft_model_state_dict
...
def main():
    ...
    logger.info("Loaded models and schedulers.")
    logger.info(f"Output directory: {args.output_dir}")
    logger.info(f"Apply patch with flip: {args.flip}, include temporal: {args.include_temporal}, include sptial: {not args.without_spatial}")
    patch.apply_patch(unet, flip = args.flip, with_temporal_block = args.include_temporal, with_spatial_block=not args.without_spatial)
    ...
    #     state_dict = {}
    #     with safe_open(os.path.join(input_dir, "model.safetensors"), framework="pt", device=0) as f:
    #         for k in f.keys():
    ...
    if accelerator.is_main_process:
        ...
        rec_txt1 = open('train_svd_trans_lora.txt', 'w')
        rec_txt2 = open('train_svd_trans_lora_train.txt', 'w')
        for name, para in unet.named_parameters():
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(cur_train_dataloader):
            ...
            with accelerator.accumulate(unet):
                ...
                video_former, video_latter = pixel_values[:, :video_len // 2], pixel_values[:, video_len // 2:]
                pixel_values = torch.cat([video_former, video_latter], dim = 0)
                ...
                # stime = time.time()
                # motion_bucket_ids = cal_motion_bucket_ids(batch["pixel_values"], batch["fps"])
                # etime = time.time()
                ...
                # video_latents = rearrange(video_latents, 'b (s t) c h w -> b t h w')
                ...
                # pixel_values = torch.cat([pixel_values, pixel_values.flip(dims = [1])], dim = 0)
                ...
                if accelerator.is_main_process:
                    if (
                        ...
                        with autocast_ctx:
                            ...
                            start_frame = T.ToPILImage()(validation_images[0])
                            end_frame = T.ToPILImage()(validation_images[num_frames // 2])
                            videos = pipeline(
                        ...
                        patch.set_joint_attention_mask(unet, [0,1])
                        videof, videob = videos



                        save_gifs_side_by_side(
                            [validation_images, videof, videob], 
                            val_save_dir
                        )
