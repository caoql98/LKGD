...
def main():
    ...
    unet.mask_embedding.requires_grad_(True)
    ...
    def collate_fn(examples):
        ...
        return {"x_pixel_values": x_pixel_values, "input_ids": input_ids, "y_pixel_values": y_pixel_values}
    ...
    logger.info(f" Grid size {grid_size} and noise patch size {noise_patch_size}")
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                xy_pixel_values = torch.cat([batch["x_pixel_values"], batch["x_pixel_values"]], dim=0)
                ...
                rand_mask = get_guassian_2d_rand_mask(grid_size, noise_patch_size)
                rand_mask = rand_mask[None,None,:latents.shape[-2],:latents.shape[-1]].expand_as(latents[bsz // 2:])
                rand_mask = torch.cat([torch.zeros_like(rand_mask), rand_mask], dim = 0).to(latents)
                # latents[bsz // 2:, ..., rand_mask] = unet.mask_embedding[None, :, None].to(dtype=weight_dtype)
                ...
                model_pred = unet(noisy_latents, timesteps, encoder_hidden_states, return_dict=False, mask = rand_mask)[0]
        ...
        if accelerator.is_main_process:
            if args.validation_prompt is not None and epoch % args.validation_epochs == 0:
                ...
                cond_latents = load_condition_latents(vae, args.cond_image_path, args.resolution)
                batch_size = 2
                cond_latents = torch.cat([cond_latents] * batch_size, dim=0)

                rand_mask = get_guassian_2d_rand_mask(grid_size, noise_patch_size)
                rand_mask = rand_mask[None,None,:cond_latents.shape[-2],:cond_latents.shape[-1]].expand_as(cond_latents[batch_size // 2:])
                rand_mask = torch.cat([torch.zeros_like(rand_mask), rand_mask], dim = 0).to(cond_latents)
                with autocast_ctx:
