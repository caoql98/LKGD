...
from pipeline.pipeline_stable_diffusion_condition import StableDiffusionPipelineJointControl
...
def main():
    ...
    unet.set_adapters(["xy_lora", "yx_lora"])
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                if args.rand_transform:
                    train_transforms = random_train_transforms()
                # pdb.set_trace()
                downsampled_x = F.interpolate(batch["x_pixel_values"], scale_factor=0.25)
                restores_x = F.interpolate(downsampled_x, scale_factor=4)

                xy_pixel_values = torch.cat([restores_x, batch["x_pixel_values"]], dim=0)
                xy_input_ids = torch.cat([batch["x_input_ids"], batch["x_input_ids"]], dim=0)
                ...
                # rand_mask = get_rand_masks(bsz, grid_size, noise_patch_size = noise_patch_size)
                # # rand_mask = torch.stack(rand_mask, dim = 0)
                # rand_mask = rand_mask[...,:height, :width].expand(bsz, 1, height, width).to(latents)
                zero_mask = torch.zeros(bsz, 1, height, width).to(xy_pixel_values.device)
                one_mask = torch.ones(bsz, 1, height, width).to(xy_pixel_values.device)
                condition = (torch.arange(bsz) < (bsz // 4)).view(-1, 1,1,1).to(xy_pixel_values.device)
                ...
                #     mask_mask = mask_mask.reshape(bsz, 1, 1, 1)
                ...
                #     mask_zeromask = mask_zeromask.reshape(bsz, 1, 1, 1)
                ...
                masked_pixel_values = xy_pixel_values * (rand_mask < 0.5)
                rand_mask = torch.nn.functional.interpolate(
                    rand_mask, size=(height // vae_scale_factor, width // vae_scale_factor)
                ).to(latents)
                masked_image_latents = vae.encode(masked_pixel_values.to(dtype=weight_dtype)).latent_dist.sample()
                masked_image_latents = masked_image_latents * vae.config.scaling_factor
                input_data = torch.cat([noisy_latents, rand_mask, masked_image_latents], dim = 1)
            ...
            if accelerator.sync_gradients:
                ...
                if args.validation_prompt is not None and (step % args.validation_steps == 0 or initial_step):
                    ...
                    init_image = process_frames([init_image], args.resolution, args.resolution)[0]
                    init_image_blurred = init_image.resize((args.resolution // 4, args.resolution // 4)).resize((args.resolution, args.resolution))
                    init_images = [init_image_blurred, init_image]
                    ...
                    rand_masks[:batch_size // 2] = torch.zeros_like(rand_masks[:batch_size // 2])
                    rand_masks[batch_size // 2:] = torch.ones_like(rand_masks[batch_size // 2:])
