...
def get_peft_model_state_dict(
    ...
    if config.peft_type in (PeftType.LORA, PeftType.ADALORA):
        bias = config.bias
        # Filter for both "lora_" and weights related to the adapter_name
        if bias == "none":
            to_return = {k: state_dict[k] for k in state_dict if "lora_" in k or f".{adapter_name}." in k}
        elif bias == "all":
            to_return = {k: state_dict[k] for k in state_dict if "lora_" in k or "bias" in k or f".{adapter_name}." in k}
        elif bias == "lora_only":
            to_return = {}
            for k in state_dict:
                if "lora_" in k or f".{adapter_name}." in k:
                    to_return[k] = state_dict[k]
                    bias_name = k.split("lora_")[0] + "bias"
                    if bias_name in state_dict:
                        to_return[bias_name] = state_dict[bias_name]
