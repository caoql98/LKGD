...
def open_points(i, j, video_filename, max_frame_idx, bucket_root, points_root):
    ...
    if len(all_tracks) > 0:
        ...
        visible_idxs = np.logical_and(all_visibles[i], all_visibles[j])
...
# def open_points(i, j, video_filename, max_frame_idx, bucket_root, points_root, query_frame_interval = 16):
#     all_tracks, all_visibles = [], []
#     mid_ij = (i + j) / 2
#     mid_ij_norm = mid_ij / query_frame_interval
#     first_query = int(mid_ij_norm + 0.5) * query_frame_interval
#     second_query = first_query - query_frame_interval if first_query > mid_ij_norm * query_frame_interval else first_query + query_frame_interval
#     queries = [first_query, second_query]
#     # print(queries, mid_ij_norm)




#     for query in queries:
#         fpath = os.path.join(bucket_root,points_root, video_filename, f"tracks_{str(query).zfill(5)}.npy")
#         if os.path.exists(fpath):
#             tracks = np.load(fpath)[0]
#             visibles = np.load(fpath.replace("tracks", "visibles"))[0]
#             all_tracks.append(tracks[:max_frame_idx])
#             all_visibles.append(visibles[:max_frame_idx])
#     if len(all_tracks) > 0:
#         all_tracks = np.concatenate(all_tracks, axis=1)
...
def filter_tracks(tracks, width, height, mask=None, num_points=None):
    ...
    if num_points:
        ...
        # valid_idxs = valid_idxs[random_idxs]
    return tracks
...
def sample_track(tracks, visibles = None, mask = None, num_samples = 5, reg = 0.333):
...
def draw_tracks(image_pil, tracks, line_width = 5, arrow_width = 5, alpha = 0.5, color = (0, 255, 0), target = False):
    source_points = tracks[0,...]
    target_points = tracks[1,...]

    image_pil_track = image_pil 

    for src_point, tgt_point in zip(source_points, target_points):
        # target_image_pil_points = add_transparent_point(target_image_pil_points.convert("RGBA"), tgt_point, 5, (0, 255,0), 127) 
        # print(src_point, tgt_point)
        if target:
            image_pil_track = draw_arrow(image_pil_track.convert("RGBA"), 
                tgt_point, src_point, line_width, color, int(alpha * 255), arrow_width)
        else:
            image_pil_track = draw_arrow(image_pil_track.convert("RGBA"), 
                src_point, tgt_point, line_width, color, int(alpha * 255), arrow_width)

    return image_pil_track


def add_point(image, position, point_radius, color, alpha):
...
def draw_points(source_image_pil, target_image_pil, tracks, point_radius = 5, alpha = 1.0, color = None):
    source_points = tracks[0,...]
    target_points = tracks[1,...]

    source_image_pil_track, target_image_pil_track = source_image_pil, target_image_pil

    for src_point, tgt_point in zip(source_points, target_points):
        # target_image_pil_points = add_transparent_point(target_image_pil_points.convert("RGBA"), tgt_point, 5, (0, 255,0), 127) 
        # print(src_point, tgt_point)
        if color is None:
            cur_color = (torch.rand(3) * 255).to(int).tolist()
        else:
            cur_color = color
        source_image_pil_track = add_point(source_image_pil_track.convert("RGBA"), 
            src_point, point_radius, cur_color, int(alpha * 255))

        target_image_pil_track = add_point(target_image_pil_track.convert("RGBA"), 
            tgt_point, point_radius, cur_color, int(alpha * 255))

    return source_image_pil_track, target_image_pil_track
